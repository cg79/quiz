...
module.exports = function(mongoQuery,encryption) {
    ...
    var models = {
        ...
        addQuizQuestion: function(obj, res) {
...
//http://localhost:8000/user?Name=Doe
	...
	var chat = io.sockets.on('connection', function (socket) {
		...
		// 	console.log(x);
		// }
		// var aaa = setTimeout(unu("test1"), 5*1000 );
							...
							switch(quizRooms[quizRoomToken].quiz.status)
							{
								case QuizRoomStatus.SendNextQuestion://send the next question
								{
									...
									if(quizRooms[quizRoomToken].quiz.index == quizRooms[quizRoomToken].quiz.noOfQuestions)
									{
										...
										for(var i=0;i<quizRooms[quizRoomToken].quiz.ratings.length;i++)
										{
											var query = RatingModel.find(
												{
													DomainID:quizRooms[quizRoomToken].domainRoom.domain.ID,
													UserGuid:quizRooms[quizRoomToken].quiz.ratings[i].UserGuid
												});
											...
											if(result.length ==1)
											{
												...
												RatingModel.update(
														   ...
														   {
														      AnswerCount: rating.AnswerCount + quizRooms[quizRoomToken].quiz.ratings[i].AnswerCount,
														      CorrectAnswerCount: rating.CorrectAnswerCount + quizRooms[quizRoomToken].quiz.ratings[i].CorrectAnswerCount,
														      WrongAnswerCount: rating.WrongAnswerCount + quizRooms[quizRoomToken].quiz.ratings[i].WrongAnswerCount
														   },
														   { upsert: true }
														)	
											}else{
												console.log("save rating to db  " + '\n');
													var ratingModel = new RatingModel(quizRooms[quizRoomToken].quiz.ratings[i]);
													    // Saving it to the database.  
										...
										mongoQuizModel.save(function (err) 
											{
												if (err)
													...
													{
														console.log ('SAVEDDDDDDDDDDDDDDD!')
													}
											});
									}
									...
									console.log("111111111111111111 question timer " + new Date() + quizRooms[quizRoomToken].quiz.index + " of " + quizRooms[quizRoomToken].quiz.noOfQuestions);	
									...
									console.log(quizRooms[quizRoomToken].questionTimerInterval  + "111111111111111111111111111111111111111111111111111111111111111111111111111111");
									quizRooms[quizRoomToken].questionTimer = setTimeout(questionTimerFunction(quizRoomToken), quizRooms[quizRoomToken].questionTimerInterval );
								...
								case QuizRoomStatus.SendAnswers://must send the answers back to the players
								{
									...
									//quizRooms[quizRoomToken].questionTimer =  setTimeout(questionTimerFunction(quizRoomToken),quizRooms[quizRoomToken].questionTimerAnswerInterval);
		...
		function asyncGetRandomQuizQuestions(reqData,quizRoomToken)
					{
						...
						getRandomQuizQuestions(reqData, function (error, result) {
					        ...
					        asyncProcessQuizQuestionsGetFromMongo(result,quizRoomToken);

					    });

					};

		function asyncProcessQuizQuestionsGetFromMongo(result, quizRoomToken)
					{
		...
		function getRatingsForLoggedUsersFromMongoDB(loggedUserGuids,quizRoomToken,chainFunction)
		{
			console.log("getRatingsForLoggedUsersFromMongoDB 1");
			var ratings=[];
		...
		function roomTimerFunction(quizRoomToken)
			{
					...
					getRatingsForLoggedUsersFromMongoDB(loggedUserGuids,quizRoomToken,asyncGetRandomQuizQuestions(reqData,quizRoomToken));
	...
	socket.on('getUserRatingForDomain', function(data) {
	    ...
	    var query = RatingModel.find(
      	...
      	query.exec(function(err, result) {
			...
			io.sockets.emit('onUserRatingForDomain', result);
		});
